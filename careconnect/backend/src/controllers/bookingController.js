...
export const createBooking = async (req, res) => {
  try {
    const { caregiver, startDate, endDate, startTime, endTime, selectedDates } = req.body;
    ...
    // Build bookingSlots from selectedDates if provided
    const bookingSlots = selectedDates?.length > 0
      ? selectedDates.map(date => ({ date, startTime, endTime }))
      : null;

    // Check availability
    const availabilityCheck = isCaregiverAvailable(caregiverData, startDate, endDate, startTime, endTime, bookingSlots);
...
export const updateBooking = async (req, res) => {
  try {
    ...
    if (isPendingBookingExpired(booking)) {
      await autoCancelSingleBooking(booking);
      return res.status(410).json({ success: false, message: AUTO_CANCELLATION_REASON });
    }
    ...
    if (status === 'confirmed' && booking.status !== 'confirmed') {
      ...
      if (!alreadyBooked) addBookedDate(caregiver, booking);
      await caregiver.save();
    ...
    if (status === 'cancelled' && booking.status !== 'cancelled') {
      if (isCaregiverUser && booking.status === 'confirmed') {
        if (!cancellationReason.trim()) {
          return res.status(400).json({ success: false, message: 'Cancellation reason is required for caregivers after confirmation' });
        }
        ...
        if (confirmationTime && Date.now() - new Date(confirmationTime).getTime() > 60 * 60 * 1000) {
          return res.status(403).json({ success: false, message: 'Caregivers can only cancel confirmed bookings within one hour of confirmation' });
        }
      }
...
export const cancelBooking = async (req, res) => {
  try {
    ...
    const cancellationReason =
      ...
      req.query?.cancellationReason || req.query?.reason || '';
    ...
    if (isClientUser) {
      ...
      if (booking.status !== 'pending') {
        return res.status(400).json({ success: false, message: 'Clients can only cancel pending bookings' });
      }
      const bookingAgeMs = Date.now() - new Date(booking.createdAt).getTime();
      if (bookingAgeMs > 5 * 60 * 1000) {
        return res.status(403).json({ success: false, message: 'Bookings can only be cancelled within 5 minutes of creation' });
      }
    }
    ...
    const cancelledBooking = await Booking.findByIdAndUpdate(
      req.params.id,
      { status: 'cancelled', cancelledAt: new Date(), cancelledBy: req.user?.role || 'unknown', cancellationReason: cancellationReason || '' },
      { new: true }
    );
...
export const updatePayment = async (req, res) => {
  try {
    ...
    if (bankSlipUrl) updateData.bankSlipUrl = bankSlipUrl;
    ...
    res.status(200).json({ success: true, message: 'Payment updated successfully', data: booking });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
  ...
  try {
    ...
    const bookingsWithReviewStatus = await Promise.all(
      bookings.map(async (booking) => {
        ...
        return { ...booking.toObject(), hasReview: !!review };
      })
    );
